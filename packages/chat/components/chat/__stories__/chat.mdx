import { ArgTypes, Markdown, Meta, ArgsTable, Controls } from '@storybook/blocks';
...
(VAIL) and the Carbon team to provide an easily expandable chat interface to
interact with large language models. We chose LIT web components as these can be
used directly in any framework or transpiled into React. It is part of the
Carbon labs initiative, where anyone can create and test out new ways to
interact with AI.
...
Objects are rendered in order using list of elements, subelement of type
**type** is rendered using **content** field
