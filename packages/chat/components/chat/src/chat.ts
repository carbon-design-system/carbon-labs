...
export default class C4AIChat extends LitElement {
   ...
   * @param{event} lit event sent by the the text input object within the chat
   **/
  _handleInput(event) {
  ...
  **/
  _sendInput(){
    ...
    this._messageText = "";
    this._messages.push({"text":value,"origin":this.username,"showButtons":false,"time":this._getCurrentTime()});
    ...
    this.getResults(value).then((res) => {
      console.log(res)
      let reply = this._checkForCode(res.reply.split("\nundefined")[0]);
      if(reply.length == 0 || res.reply == "undefined"){
        this._messages.push({"text":"Error reaching the server, try again","origin":this.agentname,"showButtons":false,"style":"error","time":this._getCurrentTime()});
      }else{
        for(let subreply of reply){
          this._messages.push({"text":subreply.text,"origin":this.agentname,"showButtons":false,"style":subreply.type,"time":this._getCurrentTime()});
        }
      }
   ...
   * @param{Number} index of message within the messages array
   * @param{event} lit hover event
   * @param{type} type of message being hovered upon
   **/
  _handleHoverIn(index, event, type){
    console.log(index);
    let object = event.target;
    if(object.className == "message-container"){
      let dropdown = object.parentElement.querySelector("."+type+"-dropdown");
  ...
  _handleHoverOut(index, event, type){
    ...
    if(relatedTarget && relatedTarget.className !== "user-dropdown"
   ...
   **/
  _updateScroll(){
    let scrollDiv = this.shadowRoot?.querySelector(".chat-messages");
    setTimeout(() =>{
      scrollDiv?.scrollTo({
