...
- [Overview](#overview)
  ...
  - [Tasks Controller](#tasks-controller)
    - [Button](#button)
    - [Dropdown](#dropdown)
    - [Content Switcher](#content-switcher-new)
  ...
  - [Accessibility](#accessibility)
    ...
    - [Tasks Content Switcher ARIA Label](#tasks-content-switcher-aria-label)
...
- **Content Switcher** renders **inline in the title row**, aligned to the right
  of “Welcome, {`user`}” on medium+ viewports (default styles). You can override
  placement with custom styles if needed.
...
contexts. You can render it as a **Button**, a **Dropdown**, or a **Content
Switcher**.

> **Placement**
>
> - **Dropdown / Button**: left column under the description.
> - **Content Switcher**: top row, aligned with the welcome title (right side).
...
The Content Switcher renders 2 or 3 segments that switch the active tile group.
It mirrors the dropdown API and updates `selectedTileGroup`.

- `visibleCount`: **2 or 3** (defaults to 2).
- If fewer than 2 items are available, the switcher will not render.
- By default we take the **first `visibleCount` groups** from `allTileGroups`.

```tsx
export const tasksControllerConfigContentSwitcher = {
  type: 'switcher',
  switcher: {
    allTileGroups: headerTiles,
    selectedTileGroup: headerTiles[0],
...
**settings icon** or a **“View all” ghost button**. A **Carousel pager** is
planned to page through multiple tile views (see “coming soon”).
...
#### Tasks Content Switcher ARIA Label
...
export const tasksControllerConfigContentSwitcher = {
  ...
  switcher: {
    ...
    setSelectedTileGroup: () => {},
    visibleCount: 2,
    ariaLabel: 'Header actions', // ✅ aria-label defined here
  },
};
```
