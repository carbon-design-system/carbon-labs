...
- [Overview](#overview)
  ...
  - [Accessibility](#accessibility)
    ...
    - [Carousel ARIA Labels](#carousel-aria-labels)
...
product’s value proposition and feature most relevant content for the user.
...
- Renders **inline in the title row**, aligned to the right of “Welcome,
...
- `lowContrast`: **boolean** — toggles Carbon’s low-contrast visual style for
  the switcher.
- Provide exactly **2 or 3** `items`. If fewer than 2 are available, it won’t
  render.
...
> In Storybook, use the **“Content Switcher Low Contrast”** toggle to set this
...
button** in the bottom action row. It's meant for quick access patterns like a
**settings icon** or a **"View all" ghost button**.

> **Divider:** A subtle vertical divider is always shown to the right of the
> header action to separate it from the carousel or Collapse button.
...
“View all” patterns that open a modal listing all tiles.
...
The Carousel is a separate pagination control that sits **between
`headerActionConfig` and the Collapse button** in the bottom action row. It
pages through multiple `TileGroup` entries supplied via `allTileGroups`, showing
one page at a time.

> **Left-to-right order:** `[ headerActionConfig ] [ carousel ] | [ Collapse ]`
>
> `headerActionConfig` and `carouselConfig` can be used at the same time.
...
import {
  AnimatedHeader,
  type HeaderCarouselConfig,
  type TileGroup,
} from '@carbon-labs/react-animated-header';

const journeyPages: TileGroup[] = [
  {
    id: 1,
    label: 'Page 1',
    tiles: [
...
| ---------------- | ------------------------------ | ------- | -------------------------------------------- |
| `carouselConfig` | `HeaderCarouselConfig \| null` | —       | Enables the carousel. Omit to hide controls. |

#### `HeaderCarouselConfig` shape

| Field             | Type                     | Default           | Description                                          |
| ----------------- | ------------------------ | ----------------- | ---------------------------------------------------- |
| `ariaLabel`       | `string`                 | —                 | `aria-label` for the carousel control group wrapper. |
| `prevButtonLabel` | `string`                 | `'Previous page'` | `aria-label` for the previous chevron button.        |
| `nextButtonLabel` | `string`                 | `'Next page'`     | `aria-label` for the next chevron button.            |
| `currentPage`     | `number`                 | `0`               | 0-based index of the visible page.                   |
| `onPageChange`    | `(page: number) => void` | —                 | Called when the user clicks prev / next / a dot.     |

#### Behaviour

- **Prev** chevron is `disabled` when `carouselConfig.currentPage === 0`.
- **Next** chevron is `disabled` when
  `carouselConfig.currentPage === totalPages - 1` (no looping).
- Each **dot** is a keyboard-accessible `<button>` that jumps directly to that
  page.
- A visually-hidden `aria-live="polite"` region announces the current page to
  screen readers on change.
- The entire control fades out when the header is collapsed (matching
  `headerActionConfig` behaviour).
...
export const contentSwitcherConfig = {
  ariaLabel: 'Content switcher actions', // ✅ aria-label defined here
  visibleCount: 2,
  lowContrast: true,
  items: [
    {
...
visual label isn’t descriptive enough (e.g., when using only an icon).
...
export const headerActionIconConfig = {
  ...
  ariaLabel: 'Open header controls', // ✅ aria-label defined here
  onClick: () => openControlsModal(),
};
```
...
Use the `carouselConfig` fields and the `ariaLabels` prop to customise all
carousel-related labels:

```tsx
const carouselConfig: HeaderCarouselConfig = {
  ariaLabel: 'Journey pages', // wrapper group label
  prevButtonLabel: 'Previous page', // prev chevron aria-label
  nextButtonLabel: 'Next page', // next chevron aria-label
};

const ariaLabels = {
  // ...other labels
  carouselDot: 'Page {n}', // dot button label template — {n} = 1-based page number
};
```
