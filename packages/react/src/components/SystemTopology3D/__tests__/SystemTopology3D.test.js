...
import SystemTopology3D from '../src/SystemTopology3D';
...
describe('SystemTopology3D', () => {
  ...
  it('renders the accessible topology region and 3D scene boundary', () => {
    render(
      <SystemTopology3D
        primaryLayer={[
          {
    ...
    expect(
      screen.getByRole('region', { name: '3D tenant system topology' })
    ).toBeInTheDocument();
    ...
    expect(screen.getByTestId('system-topology-scene')).toBeInTheDocument();
  });
