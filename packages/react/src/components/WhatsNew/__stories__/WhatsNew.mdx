...
#### Using TOC Components

The TOC components provide a fixed navigation sidebar with automatic active
state tracking. You can use them with or without Carbon Grid.

##### With Carbon Grid (Recommended for Responsive Layouts)
...
function DocumentationPage() {
  return (
    <>
      <Grid fullWidth>
        {/* Content: 12/16 on lg+, full width on smaller screens */}
        ...
        {/* TOC: 4/16 on lg+, hidden on smaller screens */}
...
- TOC stays fixed while content scrolls
- Automatically highlights active section based on scroll position
- Smooth scroll navigation when clicking TOC items
- Works with or without Carbon Grid

**Important Notes:**

- **Heading Structure**: Currently supports one level of headings (h2). Each
  `TocSection` must contain a visible `<h2>` element with an `id` attribute that
  matches the `href` in the corresponding `TocItem` (e.g.,
  `<h2 id="introduction">` matches `<TocItem href="#introduction">`).
- **Future Support**: Multi-level heading support (h2-h3) is planned for future
  releases.
