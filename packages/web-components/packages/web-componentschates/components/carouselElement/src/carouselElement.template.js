...
export function carouselElementTemplate(customElementClass) {
  ...
  return html`<div class="${clabsPrefix}--chat-carousel-container">
    ${carouselContent ? html` <div class="${clabsPrefix}--chat-carousel-length">
            ${carouselContent.length}
            ${carouselContent.length === 1 ? 'item' : 'items'}
          </div>
          <div class="${clabsPrefix}--chat-carousel-slides">
            ${carouselContent.map(item => item.type == 'img' ? html` <div class="${clabsPrefix}--chat-carousel-slide">
...
//# sourceMappingURL=carouselElement.template.js.map
