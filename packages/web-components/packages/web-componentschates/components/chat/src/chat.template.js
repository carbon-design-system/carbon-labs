...
export function chatTemplate(customElementClass) {
  ...
  return html`<div
    ...
    class="${clabsPrefix}--chat-container ${closed && !enableLauncher ? clabsPrefix + '--chat-closed' : closed && enableLauncher ? clabsPrefix + '--chat-launcher-container' : ''} ${enableDocking ? clabsPrefix + '--chat-docked' : ''} ${isDragging ? clabsPrefix + '--chat-docked-dragging' : ''}  ${enableFullscreen ? clabsPrefix + '--chat-fullscreen' : ''}">
    ${enableLauncher && closed ? html`
          <cds-icon-button
