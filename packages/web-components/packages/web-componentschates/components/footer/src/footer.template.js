...
export function footerTemplate(customElementClass) {
  ...
  return html`
    <div
      class="${clabsPrefix}--chat-footer-container${expandedHeight ? '-expanded' : ''}">
      ${!hideContextMessage && isPromptFocused && contextMessage ? html`
            <div
              class="${clabsPrefix}--chat-footer-menu ${clabsPrefix}--chat-footer-menu${contextMessageType === 'error' ? '-error' : contextMessageType === 'info' ? '-info' : contextMessageType === 'warning' ? '-warning' : ''}">
              <div class="${clabsPrefix}--chat-footer-menu-container">
                <div class="${clabsPrefix}--chat-footer-menu-container-item">
                  ${contextMessageType === 'error' ? html`<div
                        class="${clabsPrefix}--chat-footer-menu-container-item-icon-error">
                        ${WarningFilled16()}
                      </div>` : contextMessageType === 'info' ? html`<div
                        class="${clabsPrefix}--chat-footer-menu-container-item-icon-info">
                        ${InformationFilled16()}
                      </div>` : contextMessageType === 'warning' ? html`<div
                ...
                <div
                  class="${clabsPrefix}--chat-footer-menu-container-message${contextMessageType === 'error' ? '-error' : contextMessageType === 'info' ? '-info' : contextMessageType === 'warning' ? '-warning' : ''}">
                  ${contextMessage}
                </div>
                <div
                  class="${clabsPrefix}--chat-footer-menu-container-item-icon-${contextMessageType}">
                  ${contextMessageType === 'unknown' ? html`` : html`
                        <cds-icon-button
    ...
    slot: 'icon'
      ...
      ${showNotification ? html`
            <cds-inline-notification
              ...
              timeout="1200"
              aria-label="${renderLabel('complex-feedback-notification-close-label')}"
              low-contrast
              subtitle="${renderLabel('complex-feedback-notification-subtitle')}">
            </cds-inline-notification>
          ` : ''}
      <div
        class="${clabsPrefix}--chat-footer-prompt-items-target ${clabsPrefix}--chat-footer-prompt-items${expandedWidth ? contextMessageType ? '-expanded-error' : '-expanded' : ''} ${isPromptFocused ? clabsPrefix + '--chat-footer-prompt-focused' : ''} ${isPromptFocused && contextMessageType ? clabsPrefix + '--chat-footer-prompt-focused' + '-' + contextMessageType : ''}">
        <label
        ...
        >
        <textarea
          class="${clabsPrefix}--chat-search-query ${disableInput ? clabsPrefix + '--chat-search-query-disabled' : ''}"
          rows="1"
          ?disabled="${disableInput}"
          id="${clabsPrefix}--chat-footer-prompt-text-area"
          placeholder="${!disableInput ? inputPlaceholder ? inputPlaceholder : renderLabel('prompt-entry-placeholder') : renderLabel('prompt-loading-state-placeholder')}"
    ...
    slot: 'icon'
              ...
              ` : html`<cds-icon-button
                kind="ghost"
                align="top-right"
                class="${isListening ? clabsPrefix + '--chat-footer-button-danger' : ''}"
                aria-label="${isListening ? 'Stop listening' : 'Start listening'}"
                @keydown="${checkKeyboardEscapeB}"
                size="sm"
                @click="${isListening ? endRecording : startRecording}">
                ${isListening ? MicrophoneFilled16({
    slot: 'icon'
  }) : Microphone16({
    slot: 'icon'
  })}
                <span slot="tooltip-content"
                  >${renderLabel(isListening ? 'prompt-stop-listening' : 'prompt-start-listening')}</span
                >
              </cds-icon-button>`}
        </div>
        <div class="${clabsPrefix}--chat-footer-button">
          ${!currentlyStreaming && !(queryProcessing && enableCancellation) ? html`
                <cds-icon-button
                  kind="ghost"
                  size="sm"
                  aria-label="Send"
                  ...
                  ${messageText === '' || forceDisableInput ? Send16({
    slot: 'icon',
    ...
    class: clabsPrefix + '--chat-footer-send-active'
                  ...
                  <span slot="tooltip-content">
                    ${messageText === '' || forceDisableInput ? renderLabel('prompt-send-blocked-button') : renderLabel('prompt-send-button')}
                  </span>
                </cds-icon-button>
              ` : html`
                <cds-icon-button
