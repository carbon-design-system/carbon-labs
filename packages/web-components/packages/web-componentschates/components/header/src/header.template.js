  ...
  return html` <div
    ...
    <div
      ...
      <div class="${clabsPrefix}--chat-header-elements">
        ${!useOverflowMenu ? html` ${menuOpened ? html`
                  <div
        ...
        <div
          class="${clabsPrefix}--chat-header-elements-left ${dockingEnabled ? clabsPrefix + '--chat-header-elements-left-docked' : ''}"
          id="${clabsPrefix}--chat-header-container-target"
          data-floating-menu-container>
          ${!disableMenu && !disableHeaderButtons ? html`
                ${menuItems ? html`
                      ${useOverflowMenu ? html`
                            <cds-overflow-menu
                              ...
                              ${Menu24({
    slot: 'icon',
    id: clabsPrefix + '--chat-header-overflow-menu-icon'
  })}

                              <span slot="tooltip-content">
                                ${menuOpened ? renderLabel('header-close-menu') : renderLabel('header-open-menu')}
                              </span>
                              ...
                              <cds-overflow-menu-body
                                ...
                                ${menuItems.map((menuItem, index) => html` <cds-overflow-menu-item
                                    size=${dockingEnabled ? 'sm' : 'md'}
                                    aria-label="Menu Option ${index}"
                                    data-menuindex="${index}"
                                    role="menuitem"
    ...
    slot: 'icon'
                    ...
                    ` : html``}
              ` : html``}
          ${title ? html` <span class="${clabsPrefix}--chat-header-title">
                ${title}
              </span>` : ''}
        </div>
        ${dockingEnabled ? html`
            ...
            ${useSlug ? html`<cds-slug
                  ...
                  <div slot="body-text">
                    <div
                      ...
                      tabindex="0"
                      role="textbox">
                      ${headerSlugContent ? unsafeHTML(headerSlugContent) : 'Define your preferred tutorial/explanatory text within chat as an ai-slug-content attribute or as a composable slotted div element'}
                    </div>
                  </div>
                </cds-slug> ` : html` <cds-icon-button
                  size="sm"
                  kind="tertiary"
                  align="right"
                  role="button"
                  ...
                  aria-controls="${slugOpened ? clabsPrefix + '--chat-popup-unique-slug' : ''}"
                  label="${renderLabel(slugOpened ? 'message-undo-like-button' : 'message-like-button')}"
                  @click="${handleSlugClick}">
                  <span slot="icon" class="cds--slug__text">AI</span>
                  <span slot="tooltip-content"
                    >${renderLabel(slugOpened ? 'message-undo-like-button' : 'message-like-button')}</span
                  >
                </cds-icon-button>`}
          </div>

          ${!disableHeaderButtons ? html`
                ${!disableFullscreen ? html`
    ...
    slot: 'icon'
                                ...
                                <span slot="tooltip-content"
                                  >${renderLabel('header-enable-fullscreen')}</span
                                >
                              </cds-icon-button>
                            </div>
                          ` : html`
    ...
    slot: 'icon'
                                ...
                                <span slot="tooltip-content"
                                  >${renderLabel('header-disable-fullscreen')}</span
                                >
                              </cds-icon-button>
                            </div>
                          `}
                    ` : html``}
                ${!disableMinimize ? html`
                      ${!enableDocking ? html`
                            <div
                              class="${clabsPrefix}--chat-header-elements-icon">
                              <cds-icon-button
    ...
    ${slugOpened ? html` <clabs-chat-popup
