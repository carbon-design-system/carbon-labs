...
export function historyViewerTemplate(customElementClass) {
  ...
  return html` <div class="${clabsPrefix}--chat-history-viewer-container">
    ...
    return message ? html` <div
                    class="${clabsPrefix}--chat-history-viewer-message ${branchingIndices[parentId] > 1 ? clabsPrefix + '--chat-history-viewer-common-branch' : ''}">
                    ${message.action ? html` <div
                          class="${clabsPrefix}--chat-history-viewer-action-label">
                          ${message.action === 'regenerate' ? Renew16() : ''}
                          ${message.action === 'edit' ? Edit16() : ''}
                        </div>` : ''}
                    <div
                      class="${clabsPrefix + '--chat-history-viewer-block-content-' + (message.userSubmitted ? 'user' : 'bot')}">
                      ${debug ? html`<strong
                    ...
                    class="${clabsPrefix}--chat-history-viewer-message ${branchingIndices[parentId] > 1 ? clabsPrefix + '--chat-history-viewer-common-branch' : ''} ${clabsPrefix}--chat-history-viewer-empty"></div>`;
  })}
          </div>`)}
      </div>
    </div>
...
//# sourceMappingURL=historyViewer.template.js.map
