...
export function linkListElementTemplate(customElementClass) {
  ...
  return html`
    ...
    <div class="${clabsPrefix}--chat-link-list-container">
      ${linkList.map((linkObject, index) => index < maxItems || expanded ? html` <div class="${clabsPrefix}--chat-link-list-item">
              <div
                class="${clabsPrefix}--chat-link-list-item-text"
                @click="${handleLinkFeedback}">
                ${!disableRedirection ? html`
                      <cds-link
                        data-index="${index}"
    ...
    slot: 'icon'
      ...
      ${linkList.length > maxItems ? html`<div class="${clabsPrefix}--chat-link-list-item-control">
            <div class="${clabsPrefix}--chat-link-list-item-text">
              <cds-button
                ...
                ${expanded ? renderLabel('link-list-collapse-button') : renderLabel('link-list-view-all-button')}
                ${expanded ? ChevronUp16({
    slot: 'icon'
  }) : ChevronDown16({
    slot: 'icon'
  })}</cds-button
...
//# sourceMappingURL=linkListElement.template.js.map
