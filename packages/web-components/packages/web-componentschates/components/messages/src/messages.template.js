...
export function messagesTemplate(customElementClass) {
  ...
  return html` <div
    ...
    <div
      ...
      class="${clabsPrefix}--chat-messages-container ${streamResponses ? clabsPrefix + '--chat-messages-container-streaming' : ''} 

    ${dockingEnabled ? clabsPrefix + '--chat-messages-container-docked' : ''}">
      <slot name="message-items" @slotchange="${_handleSlotchange}">
        ${computedMessages ? html`
              ${computedMessages.map((message, index) => message.hasError ? html` <clabs-chat-message
                      ...
                      index="${index}">
                    </clabs-chat-message>` : html` <clabs-chat-message
                      raw-text="${message.text}"
                      origin="${message.origin}"
                      time-stamp="${message.time}"
                      ?user-submitted="${message.userSubmitted || message.origin === userName}"
