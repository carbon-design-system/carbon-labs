...
export function molecularElementTemplate(customElementClass) {
  ...
  return html`
    <div
      ...
      ${streaming ? html`<div class="${clabsPrefix}--chat-molecule-stream-text-container">
            <div class="${clabsPrefix}--chat-molecule-stream-text-content">
              ${smilesContent}
            </div>
          </div>` : ``}
      ${title && !(!isHovered && thumbNailMode) ? html`<div
            class="${clabsPrefix}--chat-molecule-title ${thumbNailMode ? clabsPrefix + '--chat-molecule-title-thumbnail' : ''}">
            ${title}
          </div>` : html``}
      ${disableOptions || loading || streaming ? html`` : html` <div
            class="${clabsPrefix}--chat-molecule-options ${thumbNailMode && !isHovered ? clabsPrefix + '--chat-molecule-options-hidden' : ''}">
    ...
    slot: 'icon'
                  ...
                  ` : html``}
              ${!disableCodeInspector && pubChemUrl ? html`
                    <cds-icon-button
                      kind="ghost"
                      size="sm"
...
//# sourceMappingURL=molecularElement.template.js.map
