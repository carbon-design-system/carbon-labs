...
export function popupElementTemplate(customElementClass) {
  ...
  return html` <div class="${clabsPrefix}--chat-popup-modal">
    ${orientation === 'top' ? html` <div class="${clabsPrefix}--chat-popup-caret-${orientation}">
          <svg
            width="22"
            height="22"
    ...
    <div
      ...
      <div class="${clabsPrefix}--chat-popup-main-content">
        ${popupTitle ? html`
            ...
            ` : ''}
        ${promptTitle ? html` <h2 class="${clabsPrefix}--chat-popup-prompt">
              ${promptTitle}
            </h2>` : ''}
        ${description ? html`<div class="${clabsPrefix}--chat-popup-description">
              ${description}
            </div>` : ''}
        ...
        ${tagList ? html`
              <div class="${clabsPrefix}--chat-popup-tag-list">
                <clabs-chat-tag-list
                  ...
                  content="${tagList ? tagList : '["Accurate","Comprehensive","Consise","Easy to understand"]'}">
                </clabs-chat-tag-list>
              </div>
            ` : ''}
        ${!disableTextArea ? html`
              <div class="${clabsPrefix}--chat-popup-feedback-text">
                <cds-textarea
                  placeholder="${textAreaPlaceholder ? textAreaPlaceholder : 'Add a comment'}"
                  rows="3"
                  cols="50"
        ...
        ${model ? html`
            ...
            ` : ''}
        ${enableDataCollectionCheck ? html`
    ...
    slot: 'icon'
      ...
      <div class="${clabsPrefix}--chat-popup-submit">
        <cds-button
          ...
          tooltip-text=" ${invalidEntry ? renderLabel('feedback-submit-button-unavailable') : renderLabel('feedback-submit-button')}"
    ...
    ${orientation === 'bottom' ? html` <div class="${clabsPrefix}--chat-popup-caret-${orientation}">
          <svg
            width="22"
            height="11"
            aria-label="caret container"
...
//# sourceMappingURL=popupElement.template.js.map
