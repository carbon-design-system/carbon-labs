...
export function tagListElementTemplate(customElementClass) {
  ...
  return html`<div class="${clabsPrefix}--chat-tag-list">
    ${invalid ? html`<div class="${clabsPrefix}--chat-tag-list-error">
          ${errorMessage}
        </div>` : html`<div class="${clabsPrefix}--chat-tag-list-container" role="group">
          ${tagList.map((value, index) => html` <div
                class="${clabsPrefix}--chat-tag-list-container-cell${isInLine ? '-inline' : ''}">
                <cds-chat-button
                  ...
                  aria-label="${value + ' ' + (selectionIndex[index] ? 'selected' : 'unselected')}"
                  ?is-selected="${selectionIndex[index]}"
                  data-content="${value}"
                  data-index="${index}"
                  @click="${handleTagClick}">
                  ${monoLabel ? monoLabel : value}
                  ${actionIcon === 'add' ? Add16({
    slot: 'icon'
  }) : actionIcon === 'upload' ? Upload16({
    slot: 'icon'
  }) : actionIcon === 'send' ? SendAlt16({
    slot: 'icon'
  }) : ''}
                </cds-chat-button>
              </div>`)}
...
//# sourceMappingURL=tagListElement.template.js.map
