  ...
  return html`<div
    ...
    <div
      ...
      <div class="${clabsPrefix}--chat-text-content">
        ${textElements.map((textPiece, index) => html` ${textPiece.type === 'annotation' || textPiece.type === 'link' ? html`
                  <span
                    class="${clabsPrefix}--chat-text-content-${textPiece.type} ${streaming ? clabsPrefix + '--chat-text-fade-in' : ''} ${enableTextHighlighting ? clabsPrefix + '--chat-text-highlighted' : textPiece.active ? clabsPrefix + '--chat-text-highlight-active' : ''} "
                    style="${textPiece.color ? 'background-color:' + textPiece.color + ';' : html``}"
                    data-index="${index}"
                    data-source="${textPiece.content}"
                    @click="${_handleAnnotationClick}">
                    ${textPiece.text}
                  </span>
                  ${!disableChevrons ? html`
                        <span
                          ...
                          style="${textPiece.color ? 'background-color:' + textPiece.color + ';' : html``}">
                          <span
                            class="${clabsPrefix}--chat-text-content-chevron ${index === annotationIndex ? clabsPrefix + '--chat-text-content-chevron--focused' : ''}"
                            style="${textPiece.color ? 'background-color:' + textPiece.color + ';' : html``}"
                            data-index="${index}"
                            data-source="${textPiece.content}"
                            @click="${_handleAnnotationClick}">
                            ${!textPiece.active ? html` ${ChevronDown16({
    slot: 'icon'
  })} ` : html` ${ChevronUp16({
    slot: 'icon'
  })} `}
                  ...
                  ${index === annotationIndex && !enableSummarization ? html`
                      ...
                      ` : html``}
                ` : alignRight ? html` <span
                  class="${streaming ? clabsPrefix + '--chat-text-fade-in' : ''}">
                  ${textPiece.text}</span
                >` : enableHtmlRendering || textPiece.type === 'html' ? html` <span
                  class="${clabsPrefix}--chat-text-content-${textPiece.type}"
                  >${unsafeHTML(textPiece.text)}</span
                >` : textPiece.type === 'new-line' ? html`<br />` : textPiece.type === 'code' ? html`
                  <span
                    class="${clabsPrefix}--chat-text-content-${textPiece.type}"
                    >${textPiece.text}</span
                  >
                ` : textPiece.type === 'default' ? html` <span
                  class="${streaming ? clabsPrefix + '--chat-text-fade-in' : ''}"
                  >${textPiece.text}</span
                >` : html`<p class="${clabsPrefix}--chat-text-content-paragraph">
                  ${textPiece.text}
                </p>`}`)}
        ${enableSummarization ? html`
              <span class="${clabsPrefix}--chat-text-content-chevron-container">
                <span
                  class="${clabsPrefix}--chat-text-content-chevron ${showSummarization ? clabsPrefix + '--chat-text-content-chevron--focused' : ''}"
                  @click="${_toggleSummarization}"
                  role="button"
                  aria-label="show all links as a carousel below">
                  ${!showSummarization ? html` ${ChevronDown16({
    slot: 'icon'
  })} ` : html` ${ChevronUp16({
    slot: 'icon'
  })} `}
...
//# sourceMappingURL=textElement.template.js.map
