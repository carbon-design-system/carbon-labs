...
const _host =
//@ts-ignore
import.meta.env && import.meta.env.VITE_SEARCH_TYPEAHEAD_API || 'https://www-api.ibm.com';
/**
 * Search endpoint
 *
 * @type {string}
...
class SearchTypeaheadAPI {
  ...
  static async getResults(query) {
    const urlQuery = [`lang=en&cc=us`, `query=${encodeURIComponent(query)}`].filter(item => item).join('&');
    const url = `${_endpoint}?${urlQuery}`;
    return await fetch(url, {
      headers: {
        'Content-Type': 'application/json; charset=utf-8'
      }
    }).then(response => response.json()).then(response => response.response);
  }
}
export default SearchTypeaheadAPI;
//# sourceMappingURL=SearchTypeahead.js.map
