...
function openFeedbackFlag(customElementClass) {
  ...
  return html`
    <div
      @mousedown=${toggle}
      class="feedback-flag"
      style="top: ${pageY - 40}px; left: ${pageX}px; font-weight: bold; display: ${isModelOpen ? 'none' : 'block'}">
      <slot name="icon"></slot>
    </div>
  `;
}
        ...
        ${isEditable ? html`
              <div style="margin-block-end:0.5rem">
                <cds-form-group style="margin-bottom-end:0.5rem">
                  <cds-text-input
                    label="Improved text"
                    value=${formData.suggestedText ? formData.suggestedText : formData.selectedText}
...
function renderIcons(feedback, position, customElementClass) {
  ...
  return html`
    <span
      ...
      style="position:absolute; top: ${(position === null || position === void 0 ? void 0 : position.bottom) - (position === null || position === void 0 ? void 0 : position.height) + 5}px; left: ${(position === null || position === void 0 ? void 0 : position.left) + (position === null || position === void 0 ? void 0 : position.width)}px;"></span>
  `;
}
/**
  ...
  return html`
    <div
      ...
      <div class="slug" @click=${resetSelection}>
        ${feedbackList.length !== 0 ? html`<span class="notify"></span>` : null}${showSlug(customElementClass)}
      </div>
    </div>
    ${Selection ? openFeedbackFlag(customElementClass) : null}
    <div id="container">
      ${feedbackList.length !== 0 && textPositions.length !== 0 ? feedbackList.map((feedback, index) => renderIcons(feedback, textPositions[index], customElementClass)) : null}
      ${highlighted ? highlighted : html`<slot
