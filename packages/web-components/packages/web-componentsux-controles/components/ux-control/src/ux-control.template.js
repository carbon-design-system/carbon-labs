...
function getHTMLRows(customElementClass) {
  ...
  return html`
    ${promptSamples.map(item => html`<cds-table-row>
          <cds-table-cell
            >${item.input.input}
            ${Object.keys(item.input.context_variables).length > 0 ? html`${Object.entries(item.input.context_variables).map(item => item.length > 0 ? html`
          ...
          <cds-table-cell
            >${item.output.view_id}
            ${Object.keys(item.output.parameters).length > 0 ? html`${Object.entries(item.output.parameters).map(item => item.length > 0 ? html`
                        <cds-tooltip align="bottom">
                          <div
                            class="sb-tooltip-trigger"
                            aria-labelledby="content">
...
function getEditModal(customElementClass) {
  ...
  if (isNewPrompt) {
    ...
    contextVariables = currentView.contextVariables.length <= 0 ? html`<div>
            This intent/view does not provide any context variables.
          </div>` : currentView.contextVariables.map(variable => html` <cds-text-input
              class="${clabsPrefix}--edit-form-item ${clabsPrefix}--edit-context-variable"
              label=${variable}
              invalid-text="Error message"
              placeholder="Enter sample value...">
            </cds-text-input>`);
    parameters = currentView.parameters.length <= 0 ? html`<div>This intent/view does not provide any parameters.</div>` : currentView.parameters.map(parameter => html` <cds-text-input
    ...
    contextVariables = Object.keys(currentContextVariables).length <= 0 ? html`<div>
            This intent/view does not provide any context variables.
          </div>` : Object.entries(currentContextVariables).map(([key, value]) => html` <cds-text-input
              class="${clabsPrefix}--edit-form-item ${clabsPrefix}--edit-context-variable"
              label=${key}
              invalid-text="Error message"
              placeholder="Enter sample value..."
    ...
    parameters = Object.keys(currentParameters).length <= 0 ? html`<div>This intent/view does not provide any parameters.</div>` : Object.entries(currentParameters).map(([key, value]) => html` <cds-text-input
              class="${clabsPrefix}--edit-form-item ${clabsPrefix}--edit-parameter"
              label=${key}
              invalid-text="Error message"
              placeholder="Enter expected value..."
  ...
  return html` <div class="${clabsPrefix}--ux-control">
        ...
        <cds-modal-heading class="${clabsPrefix}--heading-container">
          ...
          <div class="${clabsPrefix}--header-view-info">
            <div class="${clabsPrefix}--header-context-variables">
              ...
              ${currentView.contextVariables.length <= 0 ? html`<div>
                    This intent/view does not provide any parameters.
                  </div>` : currentView.contextVariables.map(variable => html`<cds-tag
                      filter
                      type="gray"
                      title="${variable}"
              ...
              ${!showAddContextVariable ? html` <cds-tag
    ...
    slot: 'icon'
            ...
            <div class="${clabsPrefix}--header-parameters">
              ...
              ${currentView.parameters.length <= 0 ? html`<div>
                    This intent/view does not provide any parameters.
                  </div>` : currentView.parameters.map(parameter => html`<cds-tag
                      filter
                      type="gray"
                      title="${parameter}"
              ...
              ${!showAddParameter ? html`<cds-tag
                    class="${clabsPrefix}--add-parameter-tag"
                    @click=${toggleAddParameter}
                    type="gray"
                    >${Add16()} Add parameter
                  </cds-tag>` : html`<div class="${clabsPrefix}--enter-new">
                    <cds-form-item>
                      <cds-text-input
...
//# sourceMappingURL=ux-control.template.js.map
