/* @web/test-runner snapshot v1 */
export const snapshots = {};

snapshots["clabs-style-picker should render single variant color picker module"] = 
`<clabs-style-picker
  align="bottom"
  kind="single"
  open=""
  title="Choose color"
>
  <cds-icon-button slot="trigger">
    <span slot="tooltip-content">
      Color palette
    </span>
  </cds-icon-button>
  <clabs-style-picker-modules slot="modules">
    <clabs-style-picker-color-module
      selected-item="blue-60"
      size="sm"
      title="Color"
    >
    </clabs-style-picker-color-module>
  </clabs-style-picker-modules>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant color picker module */

snapshots["clabs-style-picker should render single variant icon picker module"] = 
`<clabs-style-picker
  align="bottom"
  kind="single"
  open=""
  title="Choose icon"
>
  <cds-icon-button slot="trigger">
    <span slot="tooltip-content">
      Color palette
    </span>
  </cds-icon-button>
  <clabs-style-picker-modules slot="modules">
    <clabs-style-picker-color-module
      selected-item="blue-60"
      size="sm"
      title="Color"
    >
    </clabs-style-picker-color-module>
  </clabs-style-picker-modules>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant icon picker module */

snapshots["clabs-style-picker should render single variant with color picker module"] = 
`<clabs-style-picker
  align="bottom"
  kind="single"
  open=""
  title="Choose color"
>
  <cds-icon-button slot="trigger">
    <span slot="tooltip-content">
      Color palette
    </span>
  </cds-icon-button>
  <clabs-style-picker-modules slot="modules">
    <clabs-style-picker-color-module
      selected-item="blue-60"
      size="sm"
      title="Color"
    >
    </clabs-style-picker-color-module>
  </clabs-style-picker-modules>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant with color picker module */

snapshots["clabs-style-picker should render single variant with icon picker module"] = 
`<clabs-style-picker
  align="bottom"
  kind="single"
  open=""
  title="Choose icon"
>
  <cds-icon-button slot="trigger">
    <span slot="tooltip-content">
      Icon picker
    </span>
  </cds-icon-button>
  <clabs-style-picker-modules slot="modules">
    <clabs-style-picker-icon-module
      selected-item="apple"
      size="sm"
      title="Icons"
    >
    </clabs-style-picker-icon-module>
  </clabs-style-picker-modules>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant with icon picker module */

snapshots["clabs-style-picker should render single variant with pictogram picker module"] = 
`<clabs-style-picker
  align="bottom"
  kind="single"
  open=""
  title="Choose pictogram"
>
  <cds-icon-button slot="trigger">
    <span slot="tooltip-content">
      Pictogram picker
    </span>
  </cds-icon-button>
  <clabs-style-picker-modules slot="modules">
    <clabs-style-picker-pictogram-module
      selected-item="austin"
      size="lg"
      title="Pictograms"
    >
    </clabs-style-picker-pictogram-module>
  </clabs-style-picker-modules>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant with pictogram picker module */

snapshots["clabs-style-picker should render single variant with color picker options"] = 
`<clabs-style-picker
  align="bottom"
  heading="Choose color"
  kind="single"
  open=""
>
  <cds-icon-button
    kind="ghost"
    slot="trigger"
  >
    <span slot="tooltip-content">
      Color palette
    </span>
  </cds-icon-button>
  <clabs-style-picker-section
    heading=""
    size="sm"
  >
    <clabs-style-picker-group heading="Yellow">
      <clabs-style-picker-option
        aria-label="Yellow 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fcf4d6"
        label="Yellow 10"
        role="option"
        tabindex="-1"
        title="Yellow 10"
        value="#fcf4d6"
      >
        <clabs-style-picker-color
          color="#fcf4d6"
          label="Yellow 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fddc69"
        label="Yellow 20"
        role="option"
        tabindex="-1"
        title="Yellow 20"
        value="#fddc69"
      >
        <clabs-style-picker-color
          color="#fddc69"
          label="Yellow 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#f1c21b"
        label="Yellow 30"
        role="option"
        tabindex="-1"
        title="Yellow 30"
        value="#f1c21b"
      >
        <clabs-style-picker-color
          color="#f1c21b"
          label="Yellow 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#d2a106"
        label="Yellow 40"
        role="option"
        tabindex="-1"
        title="Yellow 40"
        value="#d2a106"
      >
        <clabs-style-picker-color
          color="#d2a106"
          label="Yellow 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#b28600"
        label="Yellow 50"
        role="option"
        tabindex="-1"
        title="Yellow 50"
        value="#b28600"
      >
        <clabs-style-picker-color
          color="#b28600"
          label="Yellow 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#8e6a00"
        label="Yellow 60"
        role="option"
        tabindex="-1"
        title="Yellow 60"
        value="#8e6a00"
      >
        <clabs-style-picker-color
          color="#8e6a00"
          label="Yellow 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#684e00"
        label="Yellow 70"
        role="option"
        tabindex="-1"
        title="Yellow 70"
        value="#684e00"
      >
        <clabs-style-picker-color
          color="#684e00"
          label="Yellow 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#483700"
        label="Yellow 80"
        role="option"
        tabindex="-1"
        title="Yellow 80"
        value="#483700"
      >
        <clabs-style-picker-color
          color="#483700"
          label="Yellow 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#302400"
        label="Yellow 90"
        role="option"
        tabindex="-1"
        title="Yellow 90"
        value="#302400"
      >
        <clabs-style-picker-color
          color="#302400"
          label="Yellow 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Yellow 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#1c1500"
        label="Yellow 100"
        role="option"
        tabindex="-1"
        title="Yellow 100"
        value="#1c1500"
      >
        <clabs-style-picker-color
          color="#1c1500"
          label="Yellow 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Orange">
      <clabs-style-picker-option
        aria-label="Orange 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fff2e8"
        label="Orange 10"
        role="option"
        tabindex="-1"
        title="Orange 10"
        value="#fff2e8"
      >
        <clabs-style-picker-color
          color="#fff2e8"
          label="Orange 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffd9be"
        label="Orange 20"
        role="option"
        tabindex="-1"
        title="Orange 20"
        value="#ffd9be"
      >
        <clabs-style-picker-color
          color="#ffd9be"
          label="Orange 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffb784"
        label="Orange 30"
        role="option"
        tabindex="-1"
        title="Orange 30"
        value="#ffb784"
      >
        <clabs-style-picker-color
          color="#ffb784"
          label="Orange 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ff832b"
        label="Orange 40"
        role="option"
        tabindex="-1"
        title="Orange 40"
        value="#ff832b"
      >
        <clabs-style-picker-color
          color="#ff832b"
          label="Orange 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#eb6200"
        label="Orange 50"
        role="option"
        tabindex="-1"
        title="Orange 50"
        value="#eb6200"
      >
        <clabs-style-picker-color
          color="#eb6200"
          label="Orange 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ba4e00"
        label="Orange 60"
        role="option"
        tabindex="-1"
        title="Orange 60"
        value="#ba4e00"
      >
        <clabs-style-picker-color
          color="#ba4e00"
          label="Orange 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#8a3800"
        label="Orange 70"
        role="option"
        tabindex="-1"
        title="Orange 70"
        value="#8a3800"
      >
        <clabs-style-picker-color
          color="#8a3800"
          label="Orange 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#5e2900"
        label="Orange 80"
        role="option"
        tabindex="-1"
        title="Orange 80"
        value="#5e2900"
      >
        <clabs-style-picker-color
          color="#5e2900"
          label="Orange 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#3e1a00"
        label="Orange 90"
        role="option"
        tabindex="-1"
        title="Orange 90"
        value="#3e1a00"
      >
        <clabs-style-picker-color
          color="#3e1a00"
          label="Orange 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Orange 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#231000"
        label="Orange 100"
        role="option"
        tabindex="-1"
        title="Orange 100"
        value="#231000"
      >
        <clabs-style-picker-color
          color="#231000"
          label="Orange 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Red">
      <clabs-style-picker-option
        aria-label="Red 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fff1f1"
        label="Red 10"
        role="option"
        tabindex="-1"
        title="Red 10"
        value="#fff1f1"
      >
        <clabs-style-picker-color
          color="#fff1f1"
          label="Red 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffd7d9"
        label="Red 20"
        role="option"
        tabindex="-1"
        title="Red 20"
        value="#ffd7d9"
      >
        <clabs-style-picker-color
          color="#ffd7d9"
          label="Red 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffb3b8"
        label="Red 30"
        role="option"
        tabindex="-1"
        title="Red 30"
        value="#ffb3b8"
      >
        <clabs-style-picker-color
          color="#ffb3b8"
          label="Red 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ff8389"
        label="Red 40"
        role="option"
        tabindex="-1"
        title="Red 40"
        value="#ff8389"
      >
        <clabs-style-picker-color
          color="#ff8389"
          label="Red 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fa4d56"
        label="Red 50"
        role="option"
        tabindex="-1"
        title="Red 50"
        value="#fa4d56"
      >
        <clabs-style-picker-color
          color="#fa4d56"
          label="Red 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#da1e28"
        label="Red 60"
        role="option"
        tabindex="-1"
        title="Red 60"
        value="#da1e28"
      >
        <clabs-style-picker-color
          color="#da1e28"
          label="Red 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a2191f"
        label="Red 70"
        role="option"
        tabindex="-1"
        title="Red 70"
        value="#a2191f"
      >
        <clabs-style-picker-color
          color="#a2191f"
          label="Red 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#750e13"
        label="Red 80"
        role="option"
        tabindex="-1"
        title="Red 80"
        value="#750e13"
      >
        <clabs-style-picker-color
          color="#750e13"
          label="Red 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#520408"
        label="Red 90"
        role="option"
        tabindex="-1"
        title="Red 90"
        value="#520408"
      >
        <clabs-style-picker-color
          color="#520408"
          label="Red 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Red 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#2d0709"
        label="Red 100"
        role="option"
        tabindex="-1"
        title="Red 100"
        value="#2d0709"
      >
        <clabs-style-picker-color
          color="#2d0709"
          label="Red 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Magenta">
      <clabs-style-picker-option
        aria-label="Magenta 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#fff0f7"
        label="Magenta 10"
        role="option"
        tabindex="-1"
        title="Magenta 10"
        value="#fff0f7"
      >
        <clabs-style-picker-color
          color="#fff0f7"
          label="Magenta 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffd6e8"
        label="Magenta 20"
        role="option"
        tabindex="-1"
        title="Magenta 20"
        value="#ffd6e8"
      >
        <clabs-style-picker-color
          color="#ffd6e8"
          label="Magenta 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffafd2"
        label="Magenta 30"
        role="option"
        tabindex="-1"
        title="Magenta 30"
        value="#ffafd2"
      >
        <clabs-style-picker-color
          color="#ffafd2"
          label="Magenta 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ff7eb6"
        label="Magenta 40"
        role="option"
        tabindex="-1"
        title="Magenta 40"
        value="#ff7eb6"
      >
        <clabs-style-picker-color
          color="#ff7eb6"
          label="Magenta 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ee5396"
        label="Magenta 50"
        role="option"
        tabindex="-1"
        title="Magenta 50"
        value="#ee5396"
      >
        <clabs-style-picker-color
          color="#ee5396"
          label="Magenta 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#d02670"
        label="Magenta 60"
        role="option"
        tabindex="-1"
        title="Magenta 60"
        value="#d02670"
      >
        <clabs-style-picker-color
          color="#d02670"
          label="Magenta 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#9f1853"
        label="Magenta 70"
        role="option"
        tabindex="-1"
        title="Magenta 70"
        value="#9f1853"
      >
        <clabs-style-picker-color
          color="#9f1853"
          label="Magenta 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#740937"
        label="Magenta 80"
        role="option"
        tabindex="-1"
        title="Magenta 80"
        value="#740937"
      >
        <clabs-style-picker-color
          color="#740937"
          label="Magenta 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#510224"
        label="Magenta 90"
        role="option"
        tabindex="-1"
        title="Magenta 90"
        value="#510224"
      >
        <clabs-style-picker-color
          color="#510224"
          label="Magenta 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Magenta 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#2a0a18"
        label="Magenta 100"
        role="option"
        tabindex="-1"
        title="Magenta 100"
        value="#2a0a18"
      >
        <clabs-style-picker-color
          color="#2a0a18"
          label="Magenta 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Purple">
      <clabs-style-picker-option
        aria-label="Purple 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#f6f2ff"
        label="Purple 10"
        role="option"
        tabindex="-1"
        title="Purple 10"
        value="#f6f2ff"
      >
        <clabs-style-picker-color
          color="#f6f2ff"
          label="Purple 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#e8daff"
        label="Purple 20"
        role="option"
        tabindex="-1"
        title="Purple 20"
        value="#e8daff"
      >
        <clabs-style-picker-color
          color="#e8daff"
          label="Purple 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#d4bbff"
        label="Purple 30"
        role="option"
        tabindex="-1"
        title="Purple 30"
        value="#d4bbff"
      >
        <clabs-style-picker-color
          color="#d4bbff"
          label="Purple 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#be95ff"
        label="Purple 40"
        role="option"
        tabindex="-1"
        title="Purple 40"
        value="#be95ff"
      >
        <clabs-style-picker-color
          color="#be95ff"
          label="Purple 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a56eff"
        label="Purple 50"
        role="option"
        tabindex="-1"
        title="Purple 50"
        value="#a56eff"
      >
        <clabs-style-picker-color
          color="#a56eff"
          label="Purple 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#8a3ffc"
        label="Purple 60"
        role="option"
        tabindex="-1"
        title="Purple 60"
        value="#8a3ffc"
      >
        <clabs-style-picker-color
          color="#8a3ffc"
          label="Purple 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#6929c4"
        label="Purple 70"
        role="option"
        tabindex="-1"
        title="Purple 70"
        value="#6929c4"
      >
        <clabs-style-picker-color
          color="#6929c4"
          label="Purple 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#491d8b"
        label="Purple 80"
        role="option"
        tabindex="-1"
        title="Purple 80"
        value="#491d8b"
      >
        <clabs-style-picker-color
          color="#491d8b"
          label="Purple 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#31135e"
        label="Purple 90"
        role="option"
        tabindex="-1"
        title="Purple 90"
        value="#31135e"
      >
        <clabs-style-picker-color
          color="#31135e"
          label="Purple 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Purple 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#1c0f30"
        label="Purple 100"
        role="option"
        tabindex="-1"
        title="Purple 100"
        value="#1c0f30"
      >
        <clabs-style-picker-color
          color="#1c0f30"
          label="Purple 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Blue">
      <clabs-style-picker-option
        aria-label="Blue 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#edf5ff"
        label="Blue 10"
        role="option"
        tabindex="-1"
        title="Blue 10"
        value="#edf5ff"
      >
        <clabs-style-picker-color
          color="#edf5ff"
          label="Blue 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#d0e2ff"
        label="Blue 20"
        role="option"
        tabindex="-1"
        title="Blue 20"
        value="#d0e2ff"
      >
        <clabs-style-picker-color
          color="#d0e2ff"
          label="Blue 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a6c8ff"
        label="Blue 30"
        role="option"
        tabindex="-1"
        title="Blue 30"
        value="#a6c8ff"
      >
        <clabs-style-picker-color
          color="#a6c8ff"
          label="Blue 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#78a9ff"
        label="Blue 40"
        role="option"
        tabindex="-1"
        title="Blue 40"
        value="#78a9ff"
      >
        <clabs-style-picker-color
          color="#78a9ff"
          label="Blue 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#4589ff"
        label="Blue 50"
        role="option"
        tabindex="-1"
        title="Blue 50"
        value="#4589ff"
      >
        <clabs-style-picker-color
          color="#4589ff"
          label="Blue 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 60"
        aria-selected="true"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#0f62fe"
        label="Blue 60"
        role="option"
        selected=""
        tabindex="0"
        title="Blue 60"
        value="#0f62fe"
      >
        <clabs-style-picker-color
          color="#0f62fe"
          label="Blue 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#0043ce"
        label="Blue 70"
        role="option"
        tabindex="-1"
        title="Blue 70"
        value="#0043ce"
      >
        <clabs-style-picker-color
          color="#0043ce"
          label="Blue 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#002d9c"
        label="Blue 80"
        role="option"
        tabindex="-1"
        title="Blue 80"
        value="#002d9c"
      >
        <clabs-style-picker-color
          color="#002d9c"
          label="Blue 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#001d6c"
        label="Blue 90"
        role="option"
        tabindex="-1"
        title="Blue 90"
        value="#001d6c"
      >
        <clabs-style-picker-color
          color="#001d6c"
          label="Blue 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Blue 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#001141"
        label="Blue 100"
        role="option"
        tabindex="-1"
        title="Blue 100"
        value="#001141"
      >
        <clabs-style-picker-color
          color="#001141"
          label="Blue 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Cyan">
      <clabs-style-picker-option
        aria-label="Cyan 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#e5f6ff"
        label="Cyan 10"
        role="option"
        tabindex="-1"
        title="Cyan 10"
        value="#e5f6ff"
      >
        <clabs-style-picker-color
          color="#e5f6ff"
          label="Cyan 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#bae6ff"
        label="Cyan 20"
        role="option"
        tabindex="-1"
        title="Cyan 20"
        value="#bae6ff"
      >
        <clabs-style-picker-color
          color="#bae6ff"
          label="Cyan 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#82cfff"
        label="Cyan 30"
        role="option"
        tabindex="-1"
        title="Cyan 30"
        value="#82cfff"
      >
        <clabs-style-picker-color
          color="#82cfff"
          label="Cyan 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#33b1ff"
        label="Cyan 40"
        role="option"
        tabindex="-1"
        title="Cyan 40"
        value="#33b1ff"
      >
        <clabs-style-picker-color
          color="#33b1ff"
          label="Cyan 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#1192e8"
        label="Cyan 50"
        role="option"
        tabindex="-1"
        title="Cyan 50"
        value="#1192e8"
      >
        <clabs-style-picker-color
          color="#1192e8"
          label="Cyan 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#0072c3"
        label="Cyan 60"
        role="option"
        tabindex="-1"
        title="Cyan 60"
        value="#0072c3"
      >
        <clabs-style-picker-color
          color="#0072c3"
          label="Cyan 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#00539a"
        label="Cyan 70"
        role="option"
        tabindex="-1"
        title="Cyan 70"
        value="#00539a"
      >
        <clabs-style-picker-color
          color="#00539a"
          label="Cyan 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#003a6d"
        label="Cyan 80"
        role="option"
        tabindex="-1"
        title="Cyan 80"
        value="#003a6d"
      >
        <clabs-style-picker-color
          color="#003a6d"
          label="Cyan 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#012749"
        label="Cyan 90"
        role="option"
        tabindex="-1"
        title="Cyan 90"
        value="#012749"
      >
        <clabs-style-picker-color
          color="#012749"
          label="Cyan 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cyan 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#061727"
        label="Cyan 100"
        role="option"
        tabindex="-1"
        title="Cyan 100"
        value="#061727"
      >
        <clabs-style-picker-color
          color="#061727"
          label="Cyan 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Teal">
      <clabs-style-picker-option
        aria-label="Teal 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#d9fbfb"
        label="Teal 10"
        role="option"
        tabindex="-1"
        title="Teal 10"
        value="#d9fbfb"
      >
        <clabs-style-picker-color
          color="#d9fbfb"
          label="Teal 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#9ef0f0"
        label="Teal 20"
        role="option"
        tabindex="-1"
        title="Teal 20"
        value="#9ef0f0"
      >
        <clabs-style-picker-color
          color="#9ef0f0"
          label="Teal 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#3ddbd9"
        label="Teal 30"
        role="option"
        tabindex="-1"
        title="Teal 30"
        value="#3ddbd9"
      >
        <clabs-style-picker-color
          color="#3ddbd9"
          label="Teal 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#08bdba"
        label="Teal 40"
        role="option"
        tabindex="-1"
        title="Teal 40"
        value="#08bdba"
      >
        <clabs-style-picker-color
          color="#08bdba"
          label="Teal 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#009d9a"
        label="Teal 50"
        role="option"
        tabindex="-1"
        title="Teal 50"
        value="#009d9a"
      >
        <clabs-style-picker-color
          color="#009d9a"
          label="Teal 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#007d79"
        label="Teal 60"
        role="option"
        tabindex="-1"
        title="Teal 60"
        value="#007d79"
      >
        <clabs-style-picker-color
          color="#007d79"
          label="Teal 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#005d5d"
        label="Teal 70"
        role="option"
        tabindex="-1"
        title="Teal 70"
        value="#005d5d"
      >
        <clabs-style-picker-color
          color="#005d5d"
          label="Teal 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#004144"
        label="Teal 80"
        role="option"
        tabindex="-1"
        title="Teal 80"
        value="#004144"
      >
        <clabs-style-picker-color
          color="#004144"
          label="Teal 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#022b30"
        label="Teal 90"
        role="option"
        tabindex="-1"
        title="Teal 90"
        value="#022b30"
      >
        <clabs-style-picker-color
          color="#022b30"
          label="Teal 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Teal 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#081a1c"
        label="Teal 100"
        role="option"
        tabindex="-1"
        title="Teal 100"
        value="#081a1c"
      >
        <clabs-style-picker-color
          color="#081a1c"
          label="Teal 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Green">
      <clabs-style-picker-option
        aria-label="Green 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#defbe6"
        label="Green 10"
        role="option"
        tabindex="-1"
        title="Green 10"
        value="#defbe6"
      >
        <clabs-style-picker-color
          color="#defbe6"
          label="Green 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a7f0ba"
        label="Green 20"
        role="option"
        tabindex="-1"
        title="Green 20"
        value="#a7f0ba"
      >
        <clabs-style-picker-color
          color="#a7f0ba"
          label="Green 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#6fdc8c"
        label="Green 30"
        role="option"
        tabindex="-1"
        title="Green 30"
        value="#6fdc8c"
      >
        <clabs-style-picker-color
          color="#6fdc8c"
          label="Green 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#42be65"
        label="Green 40"
        role="option"
        tabindex="-1"
        title="Green 40"
        value="#42be65"
      >
        <clabs-style-picker-color
          color="#42be65"
          label="Green 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#24a148"
        label="Green 50"
        role="option"
        tabindex="-1"
        title="Green 50"
        value="#24a148"
      >
        <clabs-style-picker-color
          color="#24a148"
          label="Green 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#198038"
        label="Green 60"
        role="option"
        tabindex="-1"
        title="Green 60"
        value="#198038"
      >
        <clabs-style-picker-color
          color="#198038"
          label="Green 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#0e6027"
        label="Green 70"
        role="option"
        tabindex="-1"
        title="Green 70"
        value="#0e6027"
      >
        <clabs-style-picker-color
          color="#0e6027"
          label="Green 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#044317"
        label="Green 80"
        role="option"
        tabindex="-1"
        title="Green 80"
        value="#044317"
      >
        <clabs-style-picker-color
          color="#044317"
          label="Green 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#022d0d"
        label="Green 90"
        role="option"
        tabindex="-1"
        title="Green 90"
        value="#022d0d"
      >
        <clabs-style-picker-color
          color="#022d0d"
          label="Green 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Green 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#071908"
        label="Green 100"
        role="option"
        tabindex="-1"
        title="Green 100"
        value="#071908"
      >
        <clabs-style-picker-color
          color="#071908"
          label="Green 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Cool gray">
      <clabs-style-picker-option
        aria-label="Cool gray 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#f2f4f8"
        label="Cool gray 10"
        role="option"
        tabindex="-1"
        title="Cool gray 10"
        value="#f2f4f8"
      >
        <clabs-style-picker-color
          color="#f2f4f8"
          label="Cool gray 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#dde1e6"
        label="Cool gray 20"
        role="option"
        tabindex="-1"
        title="Cool gray 20"
        value="#dde1e6"
      >
        <clabs-style-picker-color
          color="#dde1e6"
          label="Cool gray 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#c1c7cd"
        label="Cool gray 30"
        role="option"
        tabindex="-1"
        title="Cool gray 30"
        value="#c1c7cd"
      >
        <clabs-style-picker-color
          color="#c1c7cd"
          label="Cool gray 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a2a9b0"
        label="Cool gray 40"
        role="option"
        tabindex="-1"
        title="Cool gray 40"
        value="#a2a9b0"
      >
        <clabs-style-picker-color
          color="#a2a9b0"
          label="Cool gray 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#878d96"
        label="Cool gray 50"
        role="option"
        tabindex="-1"
        title="Cool gray 50"
        value="#878d96"
      >
        <clabs-style-picker-color
          color="#878d96"
          label="Cool gray 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#697077"
        label="Cool gray 60"
        role="option"
        tabindex="-1"
        title="Cool gray 60"
        value="#697077"
      >
        <clabs-style-picker-color
          color="#697077"
          label="Cool gray 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#4d5358"
        label="Cool gray 70"
        role="option"
        tabindex="-1"
        title="Cool gray 70"
        value="#4d5358"
      >
        <clabs-style-picker-color
          color="#4d5358"
          label="Cool gray 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#343a3f"
        label="Cool gray 80"
        role="option"
        tabindex="-1"
        title="Cool gray 80"
        value="#343a3f"
      >
        <clabs-style-picker-color
          color="#343a3f"
          label="Cool gray 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#21272a"
        label="Cool gray 90"
        role="option"
        tabindex="-1"
        title="Cool gray 90"
        value="#21272a"
      >
        <clabs-style-picker-color
          color="#21272a"
          label="Cool gray 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Cool gray 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#121619"
        label="Cool gray 100"
        role="option"
        tabindex="-1"
        title="Cool gray 100"
        value="#121619"
      >
        <clabs-style-picker-color
          color="#121619"
          label="Cool gray 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Gray">
      <clabs-style-picker-option
        aria-label="Gray 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#f4f4f4"
        label="Gray 10"
        role="option"
        tabindex="-1"
        title="Gray 10"
        value="#f4f4f4"
      >
        <clabs-style-picker-color
          color="#f4f4f4"
          label="Gray 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#e0e0e0"
        label="Gray 20"
        role="option"
        tabindex="-1"
        title="Gray 20"
        value="#e0e0e0"
      >
        <clabs-style-picker-color
          color="#e0e0e0"
          label="Gray 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#c6c6c6"
        label="Gray 30"
        role="option"
        tabindex="-1"
        title="Gray 30"
        value="#c6c6c6"
      >
        <clabs-style-picker-color
          color="#c6c6c6"
          label="Gray 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#a8a8a8"
        label="Gray 40"
        role="option"
        tabindex="-1"
        title="Gray 40"
        value="#a8a8a8"
      >
        <clabs-style-picker-color
          color="#a8a8a8"
          label="Gray 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#8d8d8d"
        label="Gray 50"
        role="option"
        tabindex="-1"
        title="Gray 50"
        value="#8d8d8d"
      >
        <clabs-style-picker-color
          color="#8d8d8d"
          label="Gray 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#6f6f6f"
        label="Gray 60"
        role="option"
        tabindex="-1"
        title="Gray 60"
        value="#6f6f6f"
      >
        <clabs-style-picker-color
          color="#6f6f6f"
          label="Gray 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#525252"
        label="Gray 70"
        role="option"
        tabindex="-1"
        title="Gray 70"
        value="#525252"
      >
        <clabs-style-picker-color
          color="#525252"
          label="Gray 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#393939"
        label="Gray 80"
        role="option"
        tabindex="-1"
        title="Gray 80"
        value="#393939"
      >
        <clabs-style-picker-color
          color="#393939"
          label="Gray 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#262626"
        label="Gray 90"
        role="option"
        tabindex="-1"
        title="Gray 90"
        value="#262626"
      >
        <clabs-style-picker-color
          color="#262626"
          label="Gray 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Gray 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#161616"
        label="Gray 100"
        role="option"
        tabindex="-1"
        title="Gray 100"
        value="#161616"
      >
        <clabs-style-picker-color
          color="#161616"
          label="Gray 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Warm gray">
      <clabs-style-picker-option
        aria-label="Warm gray 10"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#f7f3f2"
        label="Warm gray 10"
        role="option"
        tabindex="-1"
        title="Warm gray 10"
        value="#f7f3f2"
      >
        <clabs-style-picker-color
          color="#f7f3f2"
          label="Warm gray 10"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 20"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#e5e0df"
        label="Warm gray 20"
        role="option"
        tabindex="-1"
        title="Warm gray 20"
        value="#e5e0df"
      >
        <clabs-style-picker-color
          color="#e5e0df"
          label="Warm gray 20"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 30"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#cac5c4"
        label="Warm gray 30"
        role="option"
        tabindex="-1"
        title="Warm gray 30"
        value="#cac5c4"
      >
        <clabs-style-picker-color
          color="#cac5c4"
          label="Warm gray 30"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 40"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ada8a8"
        label="Warm gray 40"
        role="option"
        tabindex="-1"
        title="Warm gray 40"
        value="#ada8a8"
      >
        <clabs-style-picker-color
          color="#ada8a8"
          label="Warm gray 40"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 50"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#8f8b8b"
        label="Warm gray 50"
        role="option"
        tabindex="-1"
        title="Warm gray 50"
        value="#8f8b8b"
      >
        <clabs-style-picker-color
          color="#8f8b8b"
          label="Warm gray 50"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 60"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#726e6e"
        label="Warm gray 60"
        role="option"
        tabindex="-1"
        title="Warm gray 60"
        value="#726e6e"
      >
        <clabs-style-picker-color
          color="#726e6e"
          label="Warm gray 60"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 70"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#565151"
        label="Warm gray 70"
        role="option"
        tabindex="-1"
        title="Warm gray 70"
        value="#565151"
      >
        <clabs-style-picker-color
          color="#565151"
          label="Warm gray 70"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 80"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#3c3838"
        label="Warm gray 80"
        role="option"
        tabindex="-1"
        title="Warm gray 80"
        value="#3c3838"
      >
        <clabs-style-picker-color
          color="#3c3838"
          label="Warm gray 80"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 90"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#272525"
        label="Warm gray 90"
        role="option"
        tabindex="-1"
        title="Warm gray 90"
        value="#272525"
      >
        <clabs-style-picker-color
          color="#272525"
          label="Warm gray 90"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Warm gray 100"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#171414"
        label="Warm gray 100"
        role="option"
        tabindex="-1"
        title="Warm gray 100"
        value="#171414"
      >
        <clabs-style-picker-color
          color="#171414"
          label="Warm gray 100"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
    <clabs-style-picker-group heading="Others">
      <clabs-style-picker-option
        aria-label="Others white"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#ffffff"
        label="Others white"
        role="option"
        tabindex="-1"
        title="Others white"
        value="#ffffff"
      >
        <clabs-style-picker-color
          color="#ffffff"
          label="Others white"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
      <clabs-style-picker-option
        aria-label="Others black"
        aria-selected="false"
        class="clabs--style-picker-option clabs--style-picker-option--sm"
        data-value="#000000"
        label="Others black"
        role="option"
        tabindex="-1"
        title="Others black"
        value="#000000"
      >
        <clabs-style-picker-color
          color="#000000"
          label="Others black"
        >
        </clabs-style-picker-color>
      </clabs-style-picker-option>
    </clabs-style-picker-group>
  </clabs-style-picker-section>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant with color picker options */

snapshots["clabs-style-picker should render single variant with icon picker options"] = 
`<clabs-style-picker
  align="bottom"
  heading="Choose icon"
  kind="single"
  open=""
>
  <cds-icon-button
    kind="ghost"
    slot="trigger"
  >
    <span slot="tooltip-content">
      Icon picker
    </span>
  </cds-icon-button>
  <clabs-style-picker-section
    aria-label="Icons"
    heading="Icons"
    role="listbox"
    size="sm"
  >
    <clabs-style-picker-option
      aria-label="Apple"
      aria-selected="true"
      class="clabs--style-picker-option clabs--style-picker-option--sm"
      data-value="apple"
      label="Apple"
      role="option"
      selected=""
      tabindex="0"
      title="Apple"
      value="apple"
    >
      <clabs-style-picker-icon>
        [object Object]
      </clabs-style-picker-icon>
    </clabs-style-picker-option>
    <clabs-style-picker-option
      aria-label="Corn"
      aria-selected="false"
      class="clabs--style-picker-option clabs--style-picker-option--sm"
      data-value="corn"
      label="Corn"
      role="option"
      tabindex="-1"
      title="Corn"
      value="corn"
    >
      <clabs-style-picker-icon>
        [object Object]
      </clabs-style-picker-icon>
    </clabs-style-picker-option>
    <clabs-style-picker-option
      aria-label="Fish"
      aria-selected="false"
      class="clabs--style-picker-option clabs--style-picker-option--sm"
      data-value="fish"
      label="Fish"
      role="option"
      tabindex="-1"
      title="Fish"
      value="fish"
    >
      <clabs-style-picker-icon>
        [object Object]
      </clabs-style-picker-icon>
    </clabs-style-picker-option>
  </clabs-style-picker-section>
</clabs-style-picker>
`;
/* end snapshot clabs-style-picker should render single variant with icon picker options */

