...
class CDSTableRow extends HostListenerMixin(FocusMixin(LitElement)) {
    ...
    return !selectionName
      ...
      : html`
          <div class="${prefix}--table-column-checkbox">
            ${radio
              ? html`<c4ai-radio-button data-table></c4ai-radio-button>`
              : html`<c4ai-checkbox
                  hide-label
                  ...
                  ?disabled=${disabled}
                  ?checked=${selected}
                  value=${selectionValue}></c4ai-checkbox> `}
          </div>
        `;
  }
  ...
  /**
   * `true` if this table row is placed at an even position in parent `<c4ai-table-body>`.
   * `<c4ai-table-body>` sets this property, _only_ in zebra stripe mode.
   *
   * @private
   */
  @property({ type: Boolean, reflect: true })
  ...
  /**
   * `true` if this table row is placed at an odd position in parent `<c4ai-table-body>`.
   * `<c4ai-table-body>` sets this property, _only_ in zebra stripe mode.
   *
   * @private
   */
