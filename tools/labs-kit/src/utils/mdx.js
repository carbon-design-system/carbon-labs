 ...
 */
export async function injectMdxMetadata({
  componentDir,
  componentName,
  owners,
  problemStatement,
...
async function injectMdxFile({
  ...
  if (!content.includes(SCAFFOLD_MARKER)) {
    ...
    content = content.replace(/^(## Overview)$/m, `$1\n${scaffold}`);
  }

  await writeFile(mdxPath, content, 'utf8');
}

async function injectStoriesFile({
  componentDir,
  componentName,
  storybookGroup,
}) {
  ...
  content = content.replace(/(title:\s*['"])[^/'"]+\//, `$1${storybookGroup}/`);
